...
def reorganize_NIfTI(multi_session, session, id_prefix, dicom_folder, nifti_folder, anat_folder, func_folder, fieldmap_folder, participant_00X, task_code):
	...
	if any("T1" in x for x in anat_files):  # If there are any T1 files in the anat folder
		...
		for file in anat_files_new:
			if "T1" in file and ".nii" in file:
				os.rename(anat_folder + "/%s" % file, anat_folder + "/sub-%s%s_T1w.nii.gz" % (id_prefix, participant_00X))
			if "T1" in file and ".json" in file:
				os.rename(anat_folder + "/%s" % file, anat_folder + "/sub-%s%s_T1w.json" % (id_prefix, participant_00X))
	...
	if any("T2" in x for x in anat_files):  # If there are any T2 files in the anat folder
		...
		for file in anat_files_new:
			if "T2" in file and ".nii" in file:
				os.rename(anat_folder + "/%s" % file, anat_folder + "/sub-%s%s_T2w.nii.gz" % (id_prefix, participant_00X))
			if "T2" in file and ".json" in file:
				os.rename(anat_folder + "/%s" % file, anat_folder + "/sub-%s%s_T2w.json" % (id_prefix, participant_00X))
			...
			for original_file in task_files:
				if BOLD_run in original_file:
					if ".nii" in original_file:
						os.rename(func_folder + "/%s" % original_file, func_folder + "/sub-%s%s_task-%s_bold.nii.gz" % (id_prefix, participant_00X, file_name))
					if ".json" in original_file:
						os.rename(func_folder + "/%s" % original_file, func_folder + "/sub-%s%s_task-%s_bold.json" % (id_prefix, participant_00X, file_name))
				if SBRef in original_file:
					if ".nii" in original_file:
						os.rename(func_folder + "/%s" % original_file, func_folder + "/sub-%s%s_task-%s_sbref.nii.gz" % (id_prefix, participant_00X, file_name))
					if ".json" in original_file:
						os.rename(func_folder + "/%s" % original_file, func_folder + "/sub-%s%s_task-%s_sbref.json" % (id_prefix, participant_00X, file_name))
	...
	if rsfc_files:  # if there are any resting state files in the func folder
		...
		for file in rest_bold_sorted_list:
			if len(rest_bold_sorted_list) == 1:  # if there's only one rsfc run
				os.rename(func_folder + "/%s.nii.gz" % file, func_folder + "/sub-%s%s_task-rest_bold.nii.gz" % (id_prefix, participant_00X))
				os.rename(func_folder + "/%s.json" % file, func_folder + "/sub-%s%s_task-rest_bold.json" % (id_prefix, participant_00X))
				file_split = file.split("_")
				file_sbref = file.replace(file_split[-1], str(int(file_split[-1]) - 1))  # decrement file index by 1 for SBRef file
				os.rename(func_folder + "/%s.nii.gz" % file_sbref, func_folder + "/sub-%s%s_task-rest_sbref.nii.gz" % (id_prefix, participant_00X))
				os.rename(func_folder + "/%s.json" % file_sbref, func_folder + "/sub-%s%s_task-rest_sbref.json" % (id_prefix, participant_00X))
			else:  # if there are multiple rsfc runs, include a run number
				os.rename(func_folder + "/%s.nii.gz" % file, func_folder + "/sub-%s%s_task-rest_run-0%s_bold.nii.gz" % (id_prefix, participant_00X, run))
				os.rename(func_folder + "/%s.json" % file, func_folder + "/sub-%s%s_task-rest_run-0%s_bold.json" % (id_prefix, participant_00X, run))
				file_split = file.split("_")
				file_sbref = file.replace(file_split[-1], str(int(file_split[-1])-1))  # decrement file index by 1 for SBRef file
				os.rename(func_folder + "/%s.nii.gz" % file_sbref, func_folder + "/sub-%s%s_task-rest_run-0%s_sbref.nii.gz" % (id_prefix, participant_00X, run))
				os.rename(func_folder + "/%s.json" % file_sbref, func_folder + "/sub-%s%s_task-rest_run-0%s_sbref.json" % (id_prefix, participant_00X, run))
				run += 1
		...
		if len(fieldmap_files) == 4:  # If there is only a single pair of AP/PA fieldmap files (with their .json files)
			for file in fieldmap_files:
				if ".nii" in file:
					...
					os.rename(fieldmap_folder + "/%s" % file, fieldmap_folder + "/sub-%s%s_dir-%s_epi.nii.gz" % (id_prefix, participant_00X, direction))
					os.rename(fieldmap_folder + "/%s.json" % file[:-7], fieldmap_folder + "/sub-%s%s_dir-%s_epi.json" % (id_prefix, participant_00X, direction))
